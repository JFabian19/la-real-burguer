...
export default function App() {
  ...
  return (
    <div className="max-w-md mx-auto bg-[#10100f] min-h-screen relative shadow-2xl overflow-hidden flex flex-col font-sans text-white menu-shell">
      <header className="sticky top-0 bg-[#10100f]/95 backdrop-blur-md z-50 px-5 py-4 flex justify-between items-center border-b border-[#f59e0b]/30">
        <div className="flex flex-col items-start">
          <h1 className="font-title text-[28px] text-[#f59e0b] leading-none tracking-wide">{RESTAURANTE_NAME}</h1>
          <span className="font-slogan text-[11px] text-orange-100/70 font-bold tracking-wider mt-0.5">{RESTAURANTE_SLOGAN}</span>
        </div>
        <div className="flex items-center gap-2">
          {FACEBOOK_URL && (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="w-11 h-11 bg-[#f59e0b]/10 border border-[#f59e0b]/30 rounded-full flex items-center justify-center text-[#f59e0b] cursor-pointer"
            >
              <Facebook size={22} />
            </motion.a>
          )}
          ...
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
            className="w-11 h-11 bg-[#f59e0b]/10 border border-[#f59e0b]/30 rounded-full flex items-center justify-center relative cursor-pointer"
          >
      ...
      <div className="w-full bg-[#f59e0b] py-2 overflow-hidden flex items-center border-y border-orange-300">
        <div className="animate-marquee flex gap-6 text-[#18120a] font-slogan font-black text-[11px] tracking-widest uppercase whitespace-nowrap">
          {[...Array(10)].map((_, i) => (
      ...
      <div className="px-5 pt-4">
        <motion.button 
          ...
          className="w-full bg-gradient-to-r from-[#df3d21] via-[#f59e0b] to-[#df3d21] text-white py-3 px-4 rounded-2xl flex items-center justify-center gap-2 font-bold text-[10px] sm:text-[11px] uppercase tracking-wide border border-yellow-300/70 relative overflow-hidden group text-center"
        >
      ...
      <div className="px-5 pt-4 pb-3">
        <div className="relative w-full rounded-2xl overflow-hidden shadow-xl aspect-[2.15/1] bg-[#171715] flex flex-col items-center justify-center text-center p-4 border border-dashed border-[#f59e0b]/60 menu-hero-placeholder">
          <p className="font-dish font-bold text-[#f59e0b] text-sm uppercase tracking-[0.18em]">
            Aquí iría imagen destacada
          </p>
        </div>
      </div>

      <div className="px-5 py-3 overflow-x-auto no-scrollbar border-y border-white/5">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-[11px] font-category font-semibold whitespace-nowrap transition-all duration-200 border
                ${activeCategory === cat.id
                  ? 'bg-[#f59e0b] text-[#15100a] border-[#f59e0b] shadow-md shadow-orange-500/20'
                  : 'bg-[#1b1b19] text-white/75 border-white/15 hover:border-[#f59e0b]/70 hover:text-[#f59e0b]'
                }`}
            >
              {cat.nombre}
            </button>
      ...
      <main className="flex-1 overflow-y-auto pb-32 px-5">
        {categories.map(cat => (
          <section key={cat.id} id={`cat-${cat.id}`} className="mb-10 scroll-mt-28">
            <div className="mb-4 pt-3">
              <p className="font-slogan text-[9px] font-black tracking-[0.28em] text-[#f59e0b] uppercase mb-1">Nuestra carta</p>
              <div className="flex items-center gap-2 mb-1">
                <Utensils className="text-[#f59e0b] wave-icon" size={22} />
                <h3 className="font-category font-bold text-white text-[27px] leading-none tracking-wide category-underline">
                  {cat.nombre}
                </h3>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {cat.items.map((dish, idx) => (
                <motion.div
                  key={idx}
                  whileHover={{ x: 2 }}
                  className="menu-dish-row relative bg-[#181816] rounded-2xl overflow-hidden flex min-h-[124px] border border-[#f59e0b]/35 hover:border-[#f59e0b] transition-all duration-200"
                >
                  <div className="p-4 pr-2 flex flex-col flex-1 min-w-0">
                    <div className="flex items-start gap-2">
                      <h4 className="font-dish font-black text-white text-[14px] leading-[1.05] uppercase tracking-wide">
                        {dish.nombre}
                      </h4>
                      <span className="flex-1 border-t border-dotted border-[#f59e0b]/50 mt-2.5 min-w-3"></span>
                    </div>
                    {dish.descripcion && (
                      <p className="text-[10px] text-white/60 leading-[1.3] mt-2 line-clamp-3 pr-1">
                        {dish.descripcion}
                      </p>
                    )}
                    <div className="flex-1"></div>
                    <div className="flex items-center gap-2 mt-3">
                      <span className="font-dish font-black bg-[#f59e0b] text-[#17100a] text-[13px] px-2 py-1 rounded-md whitespace-nowrap">
                        {dish.precio}
                      </span>
                      <motion.button
                        whileTap={{ scale: 0.8 }}
                        ...
                        className="w-7 h-7 bg-white/10 border border-white/20 rounded-full flex items-center justify-center text-[#f59e0b] hover:bg-[#f59e0b] hover:text-[#17100a] transition-colors duration-200 shrink-0"
                      >
                  ...
                  <div className="w-[34%] min-w-[112px] bg-gradient-to-br from-[#302411] to-[#17130d] border-l border-[#f59e0b]/40 flex flex-col items-center justify-center text-center p-3 relative overflow-hidden dish-image-placeholder">
                    <span className="font-slogan font-black text-[9px] text-[#f7bd52] uppercase tracking-[0.14em] leading-snug">
                      Aquí iría<br />imagen
                    </span>
                  </div>
                </motion.div>
        ...
        <section className="mt-8 mb-4 border border-[#f59e0b]/35 bg-[#191916] rounded-2xl p-5 text-center shadow-sm">
          <h3 className="font-title text-[#f59e0b] text-[22px] leading-tight mb-2">¿Cómo estuvo todo?</h3>
          <p className="text-[11px] text-white/60 mb-4 px-4">Ayúdanos a mejorar calificando tu experiencia con nosotros</p>
          <motion.button 
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowReviewForm(true)}
            className="bg-[#df3d21] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-md shadow-orange-900/40 flex items-center justify-center gap-2 mx-auto w-full"
          >
        ...
        <footer className="mt-8 pt-8 pb-10 border-t border-[#f59e0b]/25 flex flex-col items-center justify-center">
          <p className="font-title text-2xl text-[#f59e0b] mb-4">{RESTAURANTE_NAME}</p>
          <div className="w-32 h-32 mb-6 rounded-2xl border border-dashed border-[#f59e0b]/50 bg-[#1b1914] flex items-center justify-center text-center p-2">
            <span className="font-dish font-bold text-[10px] text-[#f59e0b] uppercase tracking-wide">Aquí iría imagen</span>
          </div>
          <p className="text-[11px] text-white/40 font-medium">© 2026 Todos los derechos reservados.</p>
        </footer>
